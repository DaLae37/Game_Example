...
)
{
	...
	for (auto it = m_aMaterials.begin(); it != m_aMaterials.end(); it++) {
		HRESULT hr = it->get()->Initailize(pDevice, pImmediateContext);
...
void Renderable::calculateNormalMapVectors()
{
	...
	const WORD* aIndices = getIndices(); m_aNormalData.resize(GetNumVertices(), NormalData());
	XMFLOAT3 tangent, bitangent;
	for (int i = 0; i < uNumFaces; i++) {
		calculateTangentBitangent
		(aVertices[aIndices[i * 3]], aVertices[aIndices[i * 3 + 1]], aVertices[aIndices[i * 3 + 2]], tangent, bitangent);
...
void Renderable::calculateTangentBitangent(const SimpleVertex& v1, const SimpleVertex& v2, const SimpleVertex& v3, XMFLOAT3& tangent, XMFLOAT3& bitangent)
{
...
void Renderable::AddMaterial(const std::shared_ptr<Material>& material)
{
	m_aMaterials.push_back(material);
}

HRESULT Renderable::SetMaterialOfMesh(const UINT uMeshIndex, const UINT uMaterialIndex)
{
