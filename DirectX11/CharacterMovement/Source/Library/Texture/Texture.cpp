...
{
	HRESULT hr = CreateWICTextureFromFile(pDevice, pImmediateContext, m_textureFilePath.c_str(), nullptr, m_textureRV.GetAddressOf());
	if (FAILED(hr))
	{
		OutputDebugString(L"Can't load texture from \"");
		OutputDebugString(m_textureFilePath.c_str());
		OutputDebugString(L"\n");
		return hr;
	}
