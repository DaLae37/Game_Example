...
LPCWSTR						g_pszWindowName = L"DirectX11";
...
CBChangeOnCameraMovement cbCOCM;
...
HRESULT InitDevice()
{
	...
	// Pixel Solid Shader 생성
	hr = g_pd3dDevice->CreatePixelShader(pPixelShaderBlob->GetBufferPointer(), pPixelShaderBlob->GetBufferSize(), nullptr, &g_pPixelSolidShader);
	...
	eye = XMVectorSet(-2.0f, 3.0f, -5.0f, 0.0f);
	...
	cbCOCM.View = XMMatrixTranspose(g_viewMatrix);
	XMStoreFloat4(&cbCOCM.CameraPosition, eye);
	g_pImmediateContext->UpdateSubresource(g_pCBView, 0, nullptr, &cbCOCM, 0, 0);
...
void Update(FLOAT deltaTime) {
	...
	g_worldMatrix *= XMMatrixRotationY(deltaTime);
}
...
void Render()
{
	...
	for (int m = 0; m < 2; m++)
	{
		...
		g_pImmediateContext->UpdateSubresource(g_pCBWorld, 0, nullptr, &cbCEF, 0, 0);
	...
	cb.World = g_worldMatrix;
	g_pImmediateContext->UpdateSubresource(g_pCBWorld, 0, nullptr, &cb, 0, 0);
	// Shader 설정
	...
	g_pImmediateContext->PSSetConstantBuffers(0, 1, &g_pCBView);
	g_pImmediateContext->PSSetConstantBuffers(2, 1, &g_pCBWorld);
