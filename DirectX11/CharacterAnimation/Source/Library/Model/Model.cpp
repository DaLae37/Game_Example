...
)
{
    ...
    {
        AnimationData data;
        data.aBoneIndices = XMUINT4(m_aBoneData.at(i).aBoneIds);
        data.aBoneWeights = XMFLOAT4(m_aBoneData.at(i).aWeights);
...
void Model::interpolateScaling(_Inout_ XMFLOAT3& outScale, _In_ FLOAT animationTimeTicks, _In_ const aiNodeAnim* pNodeAnim)
{
    ...
    UINT uScaleIndex = findPosition(animationTimeTicks, pNodeAnim);
