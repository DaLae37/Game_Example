...
class HUD(cocos.layer.Layer):
    ...
    def show_game_over(self, winner):
        w, h = cocos.director.director.get_window_size()
        game_over = cocos.text.Label(winner, font_size=50, anchor_x='center', anchor_y='center',color=(50, 50, 255, 255))
    ...
    def minimax(self, player):
        ...
        for i, move in enumerate(moves):
            ...
            if self.difficulty == 0 : #easy
                ...
                if self.boardScore(self.table) < 0 : #컴퓨터가 불리
                    scores[i] = self.minMove(boardCopy, 2, alpha, beta)
                else : #컴퓨터가 유리
                    scores[i] = self.gutsChoice(boardCopy, 2)

        maxIndex = np.argmax(scores)

        return moves[maxIndex]

    def gutsChoice(self, board, depth) :
        moves = self.getMoves(GameLayer.PERSON, board)
        scores = np.zeros(len(moves))
        if len(moves)==0:
            if depth<=self.levelDepth:
                return self.gutsChoice(board, depth+1)
            else:
        ...
        for i, move in enumerate(moves):
            ...
            if depth>=self.levelDepth:
                ...
                return self.gutsChoice(boardCopy, depth+1)
